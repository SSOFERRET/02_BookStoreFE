...
function Banner({banners}: Props) {
    ...
    return (
        <BannerStyle>
            ...
            <BannerButtonStyle>
                <button className="prev" onClick={handlePrev}><FaAngleLeft></FaAngleLeft></button>
                <button className="next" onClick={handleNext}><FaAngleRight /></button>
            </BannerButtonStyle>
            <BannerIndicatorStyle>
                {banners.map((banner, index) => (
                    <span className={index === currentIndex ? "active" : ""} onClick={() => handleIndicatorClick(index)}
                    ></span>
                ))}
            </BannerIndicatorStyle>
        </BannerStyle>
...
const BannerButtonStyle = styled.button`
    button {
        border: 0;
        width: 40px;
        height: 40px;
